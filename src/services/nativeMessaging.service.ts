...
export class NativeMessagingService {
    ...
    private async messageHandler(msg: any) {
        ...
        if (rawMessage.command === 'setupEncryption') {
            ...
            if (rawMessage.userId !== this.accountService.activeAccount.userId) {
                ipcRenderer.send('nativeMessagingReply', {command: 'wrongUserId', appId: appId});
                return;
            }

            if (await this.accountService.getSetting<boolean>(StorageKey.EnableBrowserIntegrationFingerprint)) {
                ipcRenderer.send('nativeMessagingReply', {command: 'verifyFingerprint', appId: appId});

                const fingerprint = (await this.cryptoService.getFingerprint(this.accountService.activeAccount.userId, remotePublicKey)).join(' ');
