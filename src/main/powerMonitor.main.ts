...
export class PowerMonitorMain {
    ...
    private async getVaultTimeoutOptions(): Promise<[number, string]> {
        const timeout = await this.main.accountService.getSetting<number>(StorageKey.VaultTimeout);
        const action = await this.main.accountService.getSetting<string>(StorageKey.VaultTimeoutAction);
